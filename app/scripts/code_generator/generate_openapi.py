...
入力:  data/schemas/iso8583_fields.json   (フィールド定義)
       data/schemas/openapi_base.yaml    (手動管理のベース YAML)
       src/iso8583_manager/core/models/mti.py  (MTI enum 定義)
出力:  doc/api/openapi.yaml             (生成済み最終成果物)
"""
...
# Paths
# ---------------------------------------------------------------------------
APP_DIR = Path(__file__).parent.parent.parent          # iso8583_message_generator/app
...
SCHEMAS_DIR = APP_DIR / "data" / "schemas"             # iso8583_message_generator/app/data/schemas
DOC_API_DIR = APP_DIR.parent / "doc" / "api"           # iso8583_message_generator/doc/api

FIELDS_JSON = SCHEMAS_DIR / "iso8583_fields.json"
BASE_YAML = SCHEMAS_DIR / "openapi_base.yaml"          # ソースファイル: data/schemas/ に配置
OUTPUT_YAML = DOC_API_DIR / "openapi.yaml"             # 生成成果物: doc/api/ に配置
